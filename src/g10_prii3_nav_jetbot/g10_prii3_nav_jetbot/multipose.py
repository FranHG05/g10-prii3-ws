...
class GoToPose(Node):
    ...
    def get_result_callback(self, future):
        self.get_logger().info(
            f'🟡 Punto {self.current_goal_index + 1} completado. Verificando distancia...'
        )

        time.sleep(1.0)
        if self.last_feedback_pose is None:
            self.get_logger().warn('⚠️ No se recibió feedback. Continuando...')
        else:
            ...
            if dist > self.tolerance:
                self.get_logger().warn('⚠️ Lejos del objetivo. Reintentando...')
                self.send_next_goal()
                return

        self.get_logger().info(f'✅ Punto {self.current_goal_index + 1} alcanzado.')
